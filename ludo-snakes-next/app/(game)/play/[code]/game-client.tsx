...
const COLOR_CFG: Record<string, { bg: string; border: string; text: string; dot: string; label: string }> = {
  red:    { bg:"#FFF0F7", border:"rgba(244,114,182,.3)", text:"#BE185D", dot:"#EF4444", label:"Merah"  },
  blue:   { bg:"#EFF6FF", border:"rgba(59,130,246,.3)",  text:"#1D4ED8", dot:"#3B82F6", label:"Biru"   },
  green:  { bg:"#F0FDF4", border:"rgba(34,197,94,.3)",   text:"#15803D", dot:"#22C55E", label:"Hijau"  },
  yellow: { bg:"#FFFBEB", border:"rgba(251,191,36,.3)",  text:"#92400E", dot:"#F59E0B", label:"Kuning" },
};

const FACES = ["⚀","⚁","⚂","⚃","⚄","⚅"];

export function GameClient({ roomId, roomCode, initialState, currentUserId, roomPlayers }: GameClientProps) {
  ...
  const curName   = roomPlayers.find((r) => r.user_id === curPlayer?.user_id)?.profiles?.name ?? curPlayer?.color ?? "—";
    ...
    <div className="min-h-screen" style={{ background:"var(--bg)" }}>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-3xl border p-5"
            ...
            {isMyTurn && !state.winner && (
              <p className="text-xs mt-1 animate-pulse-dot" style={{ color:"var(--pp)" }}>
                ✨ Giliranmu! Klik roll.
              </p>
            )}
          </div>
            ...
            {enriched.map((p) => {
              ...
              return (
                <div key={p.user_id}
                  ...
                  style={{ background:isTurn ? cfg.bg : "transparent",
          ...
          <div className="rounded-3xl border p-5"
            ...
            <div className="flex items-center justify-center gap-3 mb-3">
              {/* Dadu putih 1 */}
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-xl border-2 ${diceAnim ? "animate-roll" : ""}`}
                style={{ background:"#fff", borderColor:"#E5E7EB", boxShadow:"0 2px 8px rgba(0,0,0,.06)" }}>
                {FACES[(displayDice.w1 || lastRoll?.white1 || 1) - 1]}
              </div>
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-xl border-2 ${diceAnim ? "animate-roll" : ""}`}
                style={{ background:"#fff", borderColor:"#E5E7EB", boxShadow:"0 2px 8px rgba(0,0,0,.06)" }}>
                {FACES[(displayDice.w2 || lastRoll?.white2 || 1) - 1]}
              </div>
          ...
          {/* Tombol roll */}
          {!state.winner && (
            <button onClick={handleRoll} disabled={!isMyTurn || rolling}
              className="w-full py-4 rounded-2xl text-base font-semibold transition-all"
              style={{
                background: isMyTurn ? "var(--pp)" : "#F3F4F6",
                color: isMyTurn ? "#fff" : "var(--t3)",
                boxShadow: isMyTurn ? "0 6px 20px rgba(124,111,247,0.35)" : "none",
                cursor: isMyTurn ? "pointer" : "not-allowed",
              }}>
              {rolling ? "🎲 Rolling..." : isMyTurn ? "🎲 Roll Dadu!" : "⏳ Giliran orang lain..."}
            </button>
          )}
          ...
          {/* Log */}
          {events.length > 0 && (
            <div className="rounded-2xl border p-4 max-h-32 overflow-y-auto"
              style={{ background:"var(--card)", borderColor:"var(--border)" }}>
        ...
        <div className="lg:col-span-2 space-y-4">
          {state.winner && (
            <div className="rounded-3xl p-5 text-center animate-bounce-in"
